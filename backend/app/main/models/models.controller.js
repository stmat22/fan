...
angular.module('xMember').controller('ModelsEditCtrl', function ($scope, $sce, $state, perfomerService, growl, performer, Upload, $timeout) {
  ...
  $scope.performer.bankFirstName = $scope.performer.bankFirstName || $scope.performer.firstName;
  $scope.performer.bankLastName = $scope.performer.bankLastName || $scope.performer.lastName;
  ...
  if (performer.idImg2) {
    perfomerService.getIdImage(performer.idImg2)
      .then(function (resp) {
        $scope.idFile2 = resp;
      });
  }
