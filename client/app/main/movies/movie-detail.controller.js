(function (angular) {
  ...
  function MovieDetailCtrl($scope, $rootScope, growl, $sce, commentService, $state, Auth, Lightbox, settings,
    userService, saveVideoService, videoService, photoService, perfomerService, video,
    comments, SITENAME, appConfig, me) {
    $scope.video = video;
    ...
    if ($scope.isLoggedIn && video.isSaleVideo) {
      videoService.checkBuySaleVideo(video._id)
    ...
    if (video.filePath) {
      ...
      if (appConfig.enableRTMP) {
        ...
        if (!video.convertedFiles || !video.convertedFiles.length) {
          ...
          video.convertedFiles.forEach(file => sources.push({
            file: file.path,
            label: file.width + 'p'
          }));
        }
      ...
      if (form.$valid) {
        commentService.create($scope.comment)
          .then((res) => {
            ...
            $scope.err = null;
            $scope.video.stats.totalComment = $scope.video.stats.totalComment + 1;
    ...
    if ($scope.video.performer) {
      videoService.findOthersByName($scope.video._id, $scope.video.performer[0]).then(function (data) {
        $scope.videos = data;
      });
    }
    ...
    $scope.favoriteVideo = function (video) {
      if (Auth.isLoggedIn) {
        saveVideoService.favoriteVideo(video).then(function (res) {
    ...
    $scope.watchLaterVideo = function (video) {
      if (Auth.isLoggedIn) {
        saveVideoService.watchlaterVideo(video).then(function (res) {
    ...
    $scope.downloadVideo = function (video) {
      if (Auth.isLoggedIn) {
        ...
        userService.downloadVideo(video).then(function (res) {
          // growl.success("Add to watch later successfully",{ttl:3000});
        }, function (error) {
          growl.error(error.data.error, { ttl: 3000 });
        });
    ...
    if ($scope.video.photoAlbumId) {
      photoService.search({ albumId: $scope.video.photoAlbumId, take: 1000 }).then(function (photos) {
