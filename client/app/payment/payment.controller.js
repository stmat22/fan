(function (angular) {
  ...
  function PaymentCtrl($scope, growl, $state, $stateParams, Auth, paymentService, plan, ngCart, serverSetting, performer, video) {
    ...
    if (!serverSetting.ccbillEnable && !serverSetting.paypalEnable && !serverSetting.bitpayEnable && !serverSetting.skrillEnable) {
...
//      if ($stateParams.package === 'daily') {
        $scope.subscriptionType = 'monthly';
        $scope.totalPrice = performer.subscriptionMonthlyPrice;
/*      } else if ($stateParams.package === 'weekly') {
        $scope.subscriptionType = 'weekly';
        $scope.totalPrice = performer.subscriptionWeeklyPrice;
      } else {
        $scope.totalPrice = performer.subscriptionMonthlyPrice;
      }
      */
