...
function mapUserInfo(videos, cb) {
  ...
  async.auto({
    ...
    videos.forEach(video => {
      if (video.user) {
        let index = _.findIndex(users, u => u._id.toString() === video.user.toString());
        if (index > -1) {
...
class VideoController {
  ...
  static create(req, res, next) {
    VideoValidator.validateCreating(req.body).then(data => {
          ...
          return VideoBusiness.create(video, req.user)
            .then(() => {
              if (config.fileType === 's3') {
                ...
                if (video.filePath) {
                  Queue.create('GET_SIZE_AND_STORE', video.toObject()).save();
                }
                if (video.fileTrailerPath) {
                  Queue.create('UPLOAD_TRAILER_TO_STORE', video.toObject()).save();
                }
              }
      ...
      .then(video => {
        ...
        if (config.fileType === 's3') {
          ...
          if (video.convertedFiles) {
            video.convertedFiles.forEach(v => v.path = AWSS3.getSignedUrl(v.path));
          }
        }
        //map user info
        if (!video.user) {
        ...
        async.parallel([
          ...
          function (callback) {
            async.each(video.performer, function (item, callback) {
              PerformerModel.findOne({
                _id: item
  ...
  static update(req, res, next) {
    VideoValidator.validateUpdating(req.body).then(data => {
      return VideoBusiness.findOne({
        ...
        .then(video => {
          ...
          async.auto({
            ...
            fileFullPath(cb) {
              if (!req.files || !req.files.file || !req.files.file.fileFullPath) {
                return cb();
              }
            ...
            fileTrailerPath(cb) {
              if (!req.files || !req.files.file || !req.files.file.fileTrailerPath) {
                return cb();
              }
  ...
  static like(req, res, next) {
    ...
    VideoBusiness.findOne({
      ...
      if (!video) {
        return validationError(res, 404)({
          message: 'Not found'
        });
      }
      var exists = false;
      for (var i = 0; i < video.userVotes.length; i++) {
        if (video.userVotes[i].toString() == userId.toString()) {
          exists = true;
          break;
        }
      }
      ...
      like.save(function () {
        VideoBusiness.update(video).then(function (video) {
          return res.status(200).json(video);
        })
          .catch(err => {
  ...
  static folder(req, res, next) {
    if (videosFolderCache && videosFolderCacheTime && moment(videosFolderCacheTime).add(4, 'hours').isAfter(new Date())) {
  ...
  static search(req, res) {
    ...
    let start = req.query.start ? moment(req.query.start).tz('America/New_York').toDate() : moment().tz('America/New_York').startOf('month').toDate();
    let end = req.query.end ? moment(req.query.end).tz('America/New_York').toDate() : moment().tz('America/New_York').endOf('month').toDate();
    let query = {};
    let sort = {
      activeDate: -1
    };
  ...
  static getRelatedVideos(req, res) {
    ...
    async.auto({
      ...
      byName: ['video', function (result, cb) {
        let query = {
          _id: {
            $nin: excludedIds
          },
      ...
      byTags: ['byName', function (result, cb) {
      ...
      byCategories: ['byTags', function (result, cb) {
      ...
      byPerformer: ['byCategories', function (result, cb) {
      ...
      let videos = result.byName.concat(result.byTags).concat(result.byCategories).concat(result.byPerformer);
