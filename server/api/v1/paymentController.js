  ...
  async.auto({
    ...
    */
    ...
    transaction.save(function(err, saved) {
      ...
      let url = req.body.subscriptionType === 'monthly' ?
        `https://api.ccbill.com/wap-frontflex/flexforms/${flexForm}?type=performer_subscription&transactionId=${saved._id}&userId=${req.user._id}&subscriptionType=${req.body.subscriptionType}&performerId=${req.body.performerId}&initialPrice=${initialPrice}&initialPeriod=${initialPeriod}&recurringPrice=${initialPrice}&recurringPeriod=${initialPeriod}&numRebills=99&clientSubacc=${clientSubacc}&currencyCode=${currencyCode}&formDigest=${formDigest}` :
        `https://api.ccbill.com/wap-frontflex/flexforms/${flexForm}?type=performer_subscription&transactionId=${saved._id}&userId=${req.user._id}&subscriptionType=${req.body.subscriptionType}&performerId=${req.body.performerId}&initialPrice=${initialPrice}&initialPeriod=${initialPeriod}&clientSubacc=${clientSubacc}&currencyCode=${currencyCode}&formDigest=${formDigest}`;
      cb(null, {
        redirectUrl: url
      });
    });
